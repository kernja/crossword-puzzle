...
function generateEmptyGameObject()
{
    ...
    go.isLocationValid = function(x, y, letter, orientation, isFirstLetter, isLastLetter)
    {
        //horizontal
        if (orientation == 0)
        {
            ...
            if (isFirstLetter && this.isLocationOccupied(x - 1, y)) return false;
            if (isLastLetter && this.isLocationOccupied(x + 1, y)) return false;
            ...
            if (isFirstLetter && this.isLocationOccupied(x, y - 1)) return false;
            if (isLastLetter && this.isLocationOccupied(x, y + 1)) return false;
    ...
    go.testWordsAtLocation = function(x, y, letter, orientation)
    {
        ...
        for (wi = 0; wi < potentialWords.length && placedWord == false; wi++)
        {
            ...
            while (li !== -1 && placedWord == false) {
                ...
                for (var i = 0; i < word.term.length; i++)
                {
                    if (orientation == 0)
                    {
                        if (this.isLocationValid(x + offset + i, y, word.term[i], orientation, (i == 0), (i == (word.term[i].length - 1))) == false) canPlaceWord = false;
                    } else {
                        if (this.isLocationValid(x, y + offset + i, word.term[i], orientation, (i == 0), (i == (word.term[i].length - 1))) == false) canPlaceWord = false;
                    }
                }
